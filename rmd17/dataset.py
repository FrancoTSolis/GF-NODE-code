...
class RMD17Dataset():
    ...
    def sample_cfg(self):
        ...
        """
        cfg = {}
        # Remove "revised_" prefix for the molecule type comparison
        mol_type = self.molecule_type.replace("revised_", "")
        
        if mol_type == 'benzene':
            cfg['Stick'] = [(0, 1), (2, 3), (4, 5)]
        elif mol_type == 'aspirin':
            cfg['Stick'] = [(0, 2), (1, 3), (5, 6), (7, 10), (11, 12)]
        elif mol_type == 'ethanol':
            cfg['Stick'] = [(0, 1)]
        elif mol_type == 'malonaldehyde':
            cfg['Stick'] = [(1, 2)]
        elif mol_type == 'naphthalene':
            cfg['Stick'] = [(0, 1), (2, 3), (4, 9), (5, 6), (7, 8)]
        elif mol_type == 'salicylic':
            cfg['Stick'] = [(0, 9), (1, 2), (4, 5), (6, 7)]
        elif mol_type == 'toluene':
            cfg['Stick'] = [(2, 3), (5, 6), (0, 1)]
        elif mol_type == 'uracil':
            cfg['Stick'] = [(0, 1), (3, 4)]
        elif mol_type == 'azobenzene':
            cfg['Stick'] = [(0, 1), (2, 3), (4, 5), (6, 7)]
        elif mol_type == 'paracetamol':
            cfg['Stick'] = [(0, 1), (2, 3), (4, 5), (6, 7)]
        else:
            raise NotImplementedError()
...
class RMD17DynamicsDataset(RMD17Dataset):
    """
    Revised MD17 Dynamics Dataset
    ...
    def __init__(self, partition, max_samples, delta_frame, data_dir, molecule_type, num_timesteps=8, 
        ...
        # Remove "revised_" prefix for the file name
        file_molecule_type = molecule_type.replace("revised_", "")
        
        full_dir = os.path.join(data_dir, 'rmd17_' + file_molecule_type + '.npz')
        split_dir = os.path.join(data_dir, file_molecule_type + '_split.pkl')
        ...
        # In rmd17, positions are stored under the key 'coords'
        x = data['coords']
        v = x[1:] - x[:-1]
        x = x[:-1]
        ...
        try:
            ...
            _x = x[10000: -10000] if x.shape[0] > 20000 else x 

            train_idx = np.random.choice(np.arange(_x.shape[0]), size=min(500, int(train_par * _x.shape[0])), replace=False)
            flag = np.zeros(_x.shape[0])
            for _ in train_idx:
                flag[_] = 1
            rest = [_ for _ in range(_x.shape[0]) if not flag[_]]
            val_idx = np.random.choice(rest, size=min(2000, int(val_par * _x.shape[0])), replace=False)
            for _ in val_idx:
                flag[_] = 1
            rest = [_ for _ in range(_x.shape[0]) if not flag[_]]
            test_idx = np.random.choice(rest, size=min(2000, int(test_par * _x.shape[0])), replace=False)
        ...
        # In rmd17, nuclear charges are stored under 'nuclear_charges' instead of 'z'
        z = data['nuclear_charges']
        print('mol idx:', z)
        
        # Similar to MD17, filter atoms by atomic number (z > 1), i.e., exclude hydrogen atoms
        x = x[:, z > 1, ...]
        v = v[:, z > 1, ...]
        z = z[z > 1]
...
if __name__ == "__main__":
    ...
    dataset = RMD17DynamicsDataset(partition='train', max_samples=500, delta_frame=3000, molecule_type='revised_benzene', data_dir='rmd17', num_timesteps=8, uneven_sampling=False, internal_seed=None, time_ref=3000)
